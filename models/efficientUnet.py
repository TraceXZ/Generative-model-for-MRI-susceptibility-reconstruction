...
# images normalization functions
...
class PixelShuffleUpsample(nn.Module):
    ...
    def __init__(self, dim, dim_out = None):
        super().__init__()
        dim_out = default(dim_out, dim)
        conv = nn.Conv3d(dim, dim_out * 4, 1)
...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        # (1) in cascading diffusion, one concats the low resolution images, blurred, for conditioning the higher resolution synthesis
    ...
    def forward(
        ...
        *,
        ...
        assert not (self.lowres_cond and not exists(lowres_cond_img)), 'low resolution conditioning images must be present'
        ...
        # condition on input images

        assert not (self.has_cond_image ^ exists(cond_images)), 'you either requested to condition on an images on the unet, but the conditioning images is not supplied, or vice versa'

        if exists(cond_images):
            assert cond_images.shape[1] == self.cond_images_channels, 'the number of channels on the conditioning images you are passing in does not match what you specified on initialiation of the unet'
...
class Imagen(nn.Module):
    def __init__(
        ...
        unets,
        *,
        image_sizes,                                # for cascading ddpm, images size at each stage
        ...
        lowres_sample_noise_level = 0.2,            # in the paper, they present a new trick where they noise the lowres conditioning images, and at sample time, fix it to a certain level (0.1 or 0.3) - the unets are also made to be conditioned on this noise level
        per_sample_random_aug_noise_level = False,  # unclear when conditioning on augmentation noise level, whether each batch element receives a random aug noise value - turning off due to @marunine's find
        condition_on_text = True,
        auto_normalize_img = True,                  # whether to take care of normalizing the images from [0, 1] to [-1, 1] and back automatically - you can turn this off if you want to pass in the [-1, 1] ranged images yourself from the dataloader
        ...
        # while the rest of the unets are conditioned on the low resolution images produced by previous unet
        ...
        assert not exists(first(self.random_crop_sizes)), 'you should not need to randomly crop images during training for base unet, only for upsamplers - so pass in `random_crop_sizes = (None, 128, 256)` as example'
        ...
        assert lowres_conditions == (False, *((True,) * (num_unets - 1))), 'the first unet must be unconditioned (by low resolution images), and the rest of the unets must have `lowres_cond` set to True'
        ...
        # normalize and unnormalize images functions
    ...
    def p_sample_loop(
        ...
        *,
        ...
        # for initialization with an images or video
    ...
    def sample(
        ...
        # for initial images and skipping steps
        ...
        if start_at_unet_number > 1:
            ...
            assert exists(start_image_or_video), 'starting images or video must be supplied if only doing upscaling'
    ...
    def p_losses(
        ...
        **kwargs
        ...
        if exists(random_crop_size):
            ...
            # make sure low res conditioner and images both get augmented the same way
        ...
        # also noise the lowres conditioning images
...
if __name__ == '__main__':
    ...
    model = SRUnet256(dim=128).cuda()
    img = torch.rand([1, 1, 64, 64, 64]).cuda()

    import time
    start = time.time()
    for i in range(100):
        model(img, torch.tensor([1]).cuda())
