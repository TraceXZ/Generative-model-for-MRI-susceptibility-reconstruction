...
import torch
import sys
sys.path.append('/scratch//user/uqzxion3/projects/generative_models/DM/DDIM/utils')
...
class GaussianDiffusion:
    ...
    @torch.inference_mode()
    def p_sample_3d_guidance(self, denoise_fn, guidance, dipole, shape=None, device=torch.device("cpu")):

        def condition(x_t, field, d, ts):

            d = d.cpu()
            x_t = x_t.unsqueeze(-1).transpose(0, -1)
            guidance = field.unsqueeze(-1).transpose(0, -1)

            import torch.fft as fft

            x_t_freq = fft.fftn(x_t.cpu())

            guidance_freq = fft.fftn(guidance.cpu())
            sgn_dipole = torch.sgn(d)

            value_dipole = torch.abs(d)
            ...
            tkd = guidance_freq / new_dipole
            tkd[~mask] = x_t_freq[~mask]

            return fft.ifftn(tkd).cuda().real.transpose(0, -1).squeeze(-1)

        temp_guidance = F.interpolate(guidance, scale_factor=[4, 4, 1], mode='trilinear')
        guidance = guidance.transpose(0, -1).squeeze(-1)
        temp_guidance = temp_guidance.transpose(0, -1).squeeze(-1)

        B = 20

        t = torch.empty((B, ), dtype=torch.int64, device=device)

        mini_batch = guidance.shape[0]

        num_mini_batch = mini_batch // B

        x_t = torch.randn_like(temp_guidance)
        x_0 = torch.zeros_like(temp_guidance)
        ...
        for ti in range(self.timesteps - 1, -1, -1):
            ...
            for idx in range(num_mini_batch):
                ...
                start = idx * B
                end = min((idx + 1) * B, mini_batch)
                _, x_0_inner = self.p_sample_step(denoise_fn, x_t[start: end, :, :, :], t, return_pred=True)
                x_0[start: end, :, :, :] = x_0_inner

            # for DipInv
            #  cond. #1
            # from utils.condition import dip_inv_by_tkd
            # x_0 = dip_inv_by_tkd(x_0, guidance, dipole, ts=0.3, weight=1) * mask

            #  cond. #2
            from utils.condition import dip_inv_by_init, downsample_by_crop_kspace, reverse_crop, TVRegularization

            tv = TVRegularization(1e-2, dim=[3])

            x_0 = x_0.permute([1, 2, 3, 0]).unsqueeze(0)
            guidance = guidance.permute([1, 2, 3, 0]).unsqueeze(0)

            x_0 = reverse_crop(x_0, guidance, [4, 4, 1])
            # x_0 = x_0 - 0.875 * torch.autograd.grad(tv(x_0), x_0)[0]

            x_0 = x_0.squeeze(1).permute([3, 0, 1, 2])
            guidance = guidance.squeeze(1).permute([3, 0, 1, 2])

            model_mean, *_ = self.q_posterior_mean_var(x_0, x_t, t.repeat(num_mini_batch))
            model_logvar = self._extract(self.fixed_model_logvar, t.repeat(num_mini_batch), x_t)
            noise = torch.empty_like(x_t).normal_()
            nonzero_mask = (t.repeat(num_mini_batch) > 0).reshape((-1,) + (1,) * (x_t.ndim - 1)).to(x_t)
            x_t = model_mean + nonzero_mask * torch.exp(0.5 * model_logvar) * noise

            if ti <= 5 or ti % 100 == 0:
                from utils.myio import save_tensor_as_nii
                save_tensor_as_nii(x_0, 'x_' + str(ti) + '.nii')
                # save_tensor_as_nii(x * mask, 'x_' + str(ti) + '.nii')
